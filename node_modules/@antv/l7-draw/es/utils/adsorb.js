...
import { coordAll, featureCollection, getType, lineString, nearestPointOnLine, point, polygonToLine } from '@turf/turf';
 ...
 * @param position
 */
export var getAdsorbFeature = function getAdsorbFeature(adsorbDataConfig, draw, position) {
  var features = [];
  var adsorbPoints = [];
  var adsorbLines = [];
  ...
  if (features.length) {
    ...
    if (!adsorbPoints.length) {
      adsorbPoints = coordAll(featureCollection(features)).map(function (position) {
...
export var getAdsorbPoint = function getAdsorbPoint(position, points, options, scene) {
...
export var getAdsorbLine = function getAdsorbLine(position, lines, options, scene) {
